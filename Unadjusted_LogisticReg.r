...
#not considering covariates in this step
library(tidyverse)
...
glm.logistic <- function(metab,phenotype, dose, df){
  eq.1 = paste(phenotype, "~", dose)
  m.1 = glm(eq.1, family = 'binomial', data=df)
  
  eq.2 = paste(phenotype, "~", metab, "+", dose)
  ...
  eq.3 = paste(phenotype, "~", metab, "*", dose)
  m.3 = glm(eq.3, family = 'binomial', data =df)
  m.3_summary = (summary(m.3))
  BetaMD = m.3_summary$coefficients[4,1]
  BetaMD_StdError = m.3_summary$coefficients[4,2]
...
ac_logistic_results = lapply(ac_metabolites, function(x) glm.logistic(
  metab=x, phenotype ='died_90_day', dose='dose', df= ac_metabolites_matrix_scaled))

nmr_logistic_results = lapply(nmr_metabolite_names, function(x) glm.logistic(
  metab=x, phenotype ='died_90_day', dose='dose', df= nmr_metabolites_matrix_scaled))

#combine AC and NMR results
unadjusted_logistic = c(ac_logistic_results, nmr_logistic_results)
...
for (i in unadjusted_logistic){
  results = rbind(results, i)
}

#Turn columns numeric and apply a p-value correction 
unadjusted_results = data.frame(results)

unadjusted_results[,2:7] = lapply(unadjusted_results[,2:7], function(x) as.numeric(as.character(x)))
unadjusted_results$Bonferroni_Q = p.adjust(unadjusted_results$BetaM_pvalue, method = 'bonferroni')

#combine
write.csv(x = unadjusted_results, file = 'unadjusted_metab_only.csv')
